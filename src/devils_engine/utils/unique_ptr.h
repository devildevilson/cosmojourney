...
namespace devils_engine {
  namespace utils {
    ...
    class unique_ptr {
    public:
      //constexpt size_t container_size = sizeof(T);

      unique_ptr() noexcept : m_memory(nullptr) {}
      unique_ptr(std::nullptr_t) noexcept : m_memory(nullptr) {}
    ...
    private:
      //char m_memory[container_size]; // move здесь не получается, нарушается сохранность сырых указателей
      char* m_memory;
    };
  }
}
